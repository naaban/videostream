...
function serverHandler(request, response) {
    try {
        ...
        try {
            ...
            if (filename && filename.search(/html/g) === -1 && stats.isDirectory()) {
        ...
        fs.readFile(filename, 'binary', function(err, file) {
            ...
            try {
                ...
                if (html.length) {
                    ...
                    var otherHtml = '<section class="experiment" id="html"><details><summary style="text-align:center;">Check ' + (html.length - 1) + ' other RTCMultiConnection-v3 html</summary>' + h2 + '<ol>';
                    html.forEach(function(f) {
                        if (f && f !== 'index.html' && f.indexOf('.html') !== -1) {
                            otherHtml += '<li><a href="/html/' + f + '">' + f + '</a> (<a href="https://github.com/muaz-khan/RTCMultiConnection/tree/master/html/' + f + '">Source</a>)</li>';
                        }
                    });
                    otherHtml += '<ol></details></section><section class="experiment own-widgets latest-commits">';

                    file = file.replace('<section class="experiment own-widgets latest-commits">', otherHtml);
                }
            } catch (e) {}
...
function runServer() {
    ...
    app = app.listen(port, process.env.IP || '0.0.0.0', function(error) {
        ...
        if (addr.address != 'localhost' && !isUseHTTPs) {
            ...
            console.log(BASH_COLORS_HELPER.getRedBG(), 'Please run on HTTPs to make sure audio,video and screen html can work on Google Chrome as well.');
        }
